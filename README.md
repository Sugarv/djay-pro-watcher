...
2. **Open the App:** Right-click on `DjayProWatcher.app` and choose **Open**.
   You must right-click and choose Open the very first time to bypass macOS developer warnings OR open the terminal and type: `xattr -cr /path/to/your/dist/DjayProWatcher.app`.
4. **Enter API Key:** The first time you run the app, a window will pop up asking for your [Last.fm API Key](https://www.last.fm/api/account/create). Paste your key and click OK.
5. **Start Monitoring:** A DJ icon will appear in your Mac's menu bar (top right, near the clock). Click it and select **Start Monitoring**.
6. **Copy Songs:** As djay Pro plays, similar song suggestions will appear in that menu (a bell icon!). Just click on any song to instantly copy it to your clipboard!
